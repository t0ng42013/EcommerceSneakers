...
export const Hamburger = () => {
  ...
  return (
    ...
    >
      <button
        className="bg-white p-4 w-full max-w-[321px] text-start font-semibold text-gray-600 text-xl md:hidden md:w-full"
        onClick={() => dispatch(toggleMenu(menu))}
      >
        <IoClose className="text-gray hover:text-orange " />
      </button>

      <ul className=" absolute left-0 top-[40px]   z-20 bg-white h-full pl-4 pt-4 w-full max-w-[321px] md:flex md:w-full md:bg-transparent md:static md:items-center md:[&>li]:mb-0 md:gap-8 md:[&>li]:text-DarkGrayishBlue md:[&>li]:font-medium md:[&>li]:h-[100px] md:[&>li]:flex md:[&>li]:items-center md:py-0 md:hover:[&>li]:border-b-4 md:[&>li]:border-orange hover:[&>li]:text-orange md:hover:[&>li]:text-VeryDarkBlue md:[&>li]:ml-4">
